<script lang="ts">
    ...
    $: hovered_title_link = `
        <a href="${BASE_URL}/#/${params.language}/artworks/${hovered_title.id}"
    ...
    function mouse_on_title(e) {
        if (window.screen.width < 768) {
            window.location.href = `${BASE_URL}/#/${params.language}/artworks/${e.target.id}`;
            return;
        }
        ...
        word_count_interval = setInterval(() => {
            previewDOM.innerHTML = artworks_info[hovered_title.id].description.slice(0, letter_count) + (letter_count % 60 >= 30 ? "▌" : "&nbsp;");
            letter_count += 3;
        }, 50);
    }
...
<div class="pt-12">
    ...
    <div class="flex flex-wrap relative left-0 mx-20 items-center justify-center text-center">
        {#each artworks_info as team}
            <div class="p-3 {tailwind_rotates[Math.floor(Math.random() * tailwind_rotates.length)]}">
                <a
                    class="font-extrabold blur-2xl text-cyan-50/80 text-2xl hover:blur-3xl p-5 ease-in-out transition-all duration-700 cursor-pointer"
                    on:click={() => (window.location.href = `${BASE_URL}/#/${params.language}/artworks/${team.id}`)}
    ...
    <div
        class="shadow-lg shadow-black bg-black/30
        ...
        <div class="shadow-sm shadow-black border-none p-3 rounded bg-white/5 w-fit inline">
            ...
            <p class="inline">
                {#each artworks_info[hovered_title.id].members as member}
                    <a class="cursor-pointer hover:border-b-2" href="{BASE_URL}/#/{params.language}/member/{member.id}">
