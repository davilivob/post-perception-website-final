<script>
    ...
    if (external_links) {
        if (external_links.instagram) {
            external_links_content += generate_link(
                external_links.instagram,
                "fa-brands fa-instagram",
                "hover:text-fuchsia-500/30 text-fuchsia-500"
            );
        }
        if (external_links.facebook) {
            external_links_content += generate_link(
                ...
                "hover:text-sky-500/30 text-sky-500"
            );
        }
        if (external_links.youtube) {
            external_links_content += generate_link(
                ...
                "hover:text-red-700/30 text-red-700"
            );
        }
        if (external_links.github) {
            external_links_content += generate_link(
                ...
                "fa-brands fa-github",
                "hover:text-white/30 text-white"
            );
        }
        if (external_links.nft) {
            external_links_content += generate_link(
                ...
                "hover:text-violet-500/30 text-violet-500"
            );
        }
        if (external_links.website) {
            external_links_content += generate_link(
                external_links.website,
                "fa-regular fa-browser",
                "hover:text-lime-500/30 text-lime-500"
            );
        }
        if (external_links.email) {
            external_links_content += generate_link(
                ...
                "hover:text-amber-500/30 text-amber-500"
            );
        }
    }
...
<div id="main" class="flex justify-center items-center pt-12">
    <div class="flex flex-row flex-wrap">
        <div
            class="flex flex-col mx-10 mt-10 justify-center items-center gap-3"
        >
            <div
                class="fixed w-screen h-screen z-[-1] top-0 bg-repeat-y bg-cover bg-center shadow-lg"
                style="background-image: url('/images/exhibition/artwork_photos/{artwork_info.id}/0.jpg')"
            />
            ...
            {#if is_mobile}
                <FacePic
                    ...
                    rounded="full"
                    w="w-[40vh]"
                    h="max-h-[40vh] shadow-lg shadow-black/40"
            ...
            <div
                class="text-lg p-5 rounded-xl bg-black/10 shadow-2xl shadow-black/50 flex flex-col max-w-[80vw] w-fit break-all backdrop-blur-xl text-white/90"
            >
                <a
                    href="/#/{params.language}/artworks/{artwork_info.id}"
                    class="text-lg hover:text-white text-white/40 duration-200 underline"
                >
                    #{#if artwork_info.member_title}
                        {#if is_en}
                            {artwork_info.member_title.replace(
                                " ",
                                "_"
                            )}_of_{artwork_info.title.replace(" ", "_")}
                        {:else}
                ...
                {#each admins_info as admin}
                    <a
                        class="font text-lg text-white/40 underline hover:text-white"
